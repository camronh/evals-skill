...
The expanded stats panel shows the current session and run on the left, with test count, latency, and score metrics alongside the chart.

## Use Cases

<AccordionGroup>
  <Accordion title="Model Comparison">
    Compare performance across different models or configurations using run configs:
    ...
    ```bash
    ezvals run evals/ --session model-comparison --config gpt-4
    ezvals run evals/ --session model-comparison --config claude
    ```

    Each run auto-names itself after the config, and the config dict is available as `ctx.config` in your eval code. Use `ezvals serve` to compare runs side by side.
  </Accordion>
