...
<ParamField path="-o, --output" type="string">
  Override the default results path. When specified, results are saved only to this path (not to `.ezvals/sessions/`).
</ParamField>
...
ezvals run evals/ --no-save | jq '.total_passed'
```
...
```bash
# Results auto-save to .ezvals/sessions/ by default
ezvals run evals/
...
| `results_dir` | string | Base directory; runs are stored in `{results_dir}/.ezvals/sessions` | `run`, `serve` |
...
```
Running evals/...
Results saved to .ezvals/sessions/default/swift-falcon_a1b2c3d4.json
```

### JSON File Output

Results are always saved as JSON to `.ezvals/sessions/` (or custom path via `-o`):
