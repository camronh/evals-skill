...
**Multiple scoring criteria** — We use `ctx.store(scores=...)` for hallucination (a named score) and `assert` for pass (the default score). Both appear in your results.

**LLM-as-judge** — The `hallucination_judge` and `pass_judge` are placeholder functions representing whatever LLM judge you're using (OpenAI, Anthropic, your own prompts, etc.).
