...
<Note>
When a PATCH changes one of those fields, EZVals appends a `correction_history` entry with `before`/`after` values and a UTC timestamp.
</Note>
